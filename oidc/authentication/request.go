...
//
// If the ID Token received by the RP from the OP is encrypted, to use it as an id_token_hint, the Client MUST decrypt the signed ID Token contained within the encrypted ID Token. The Client MAY re-encrypt the signed ID verifier to the Authentication Server using a key that enables the server to decrypt the ID Token, and use the re-encrypted ID verifier as the id_token_hint value.
...
const (
	format = "%s?scope=openid&response_type=code&login=true&client_id=%s&redirect_uri=%s"
)

func NewRequest(authorizationEndpoint string, clientId string, redirectURI string) *Request {
	return &Request{
...
func (ar *Request) ToString() string {
	return fmt.Sprintf(format, ar.authorizationEndpoint, ar.clientId, ar.redirectURI)
}
